...
static intcode machine_read_pos(const struct machine *machine, intcode pointer) {
...
static void machine_set_pos(struct machine *machine, intcode pointer, intcode value) {
  if (0 <= pointer && (size_t)pointer < MACHINE_MEM_SIZE) {
...
static void machine_write(struct machine *m, enum param_mode mode, intcode pointer,
                   intcode value) {
  switch (mode) {
  case PARAM_POS:
...
static void run_binary_op(struct machine *machine, intcode instr, binary_operator op) {
...
static void run_jump_op(struct machine *machine, intcode instr, pred_operator op) {
  intcode param_modes = instr / 100;
...
static enum machine_state machine_step(struct machine *machine) {
  if (machine->halted) {
    return HALTED;
  }
