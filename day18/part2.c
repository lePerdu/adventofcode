...
struct node_list {
  unsigned size;
  struct robot_step data[MAX_NODE_COUNT];
};

struct search_state {
  graph_node current_nodes[4];
  unsigned total_dist;
...
static void find_keys_from_node(struct node_list *output,
                                const struct graph *graph,
                                uint32_t keys_bitmask, graph_node start_node,
                                uint8_t robot_index) {
  struct heap q;
  hinit(&q);
    ...
    for (unsigned i = 0; i <= adj_list->size; i++) {
      ...
      if (node_is_door(to_index)) {
        ...
        if (keys_bitmask & (1 << key_index)) {
          ...
          output->data[output->size++] = (struct robot_step){
              .node = to_index,
              .dist = total_dist,
              .robot = robot_index,
          };
        ...
        recurse_at_to_index = keys_bitmask & (1 << key_index);
      }
...
static void find_accessible_keys_dfs(struct node_list *output,
                                     ...
                                     const struct graph *graph) {
  output->size = 0;
  for (robot_index i = 0; i < 4; i++) {
    find_keys_from_node(output, graph, state->keys_bitmask,
                        state->current_nodes[i], i);
  }
}
...
static bool should_visit_state(struct state_cache *c,
                               const struct search_state *state) {
  // Do bitshifting to efficiently convert the state value into a single
  // uint64_t
  static_assert(sizeof(state->current_nodes) == sizeof(uint32_t),
                "Wrong number of robots");
  uint32_t current_nodes_value = *(uint32_t *)state->current_nodes;
  uint64_t value =
      (((uint64_t)current_nodes_value) << 32) + (uint64_t)state->keys_bitmask;
...
static void find_best_path(struct search_context *context,
  ...
  if (next_steps.size == 0) {
    debugf("Dead end at (%c, %c, %c, %c) (bitmask = %x)\n",
           get_node_char(state->current_nodes[0]),
           get_node_char(state->current_nodes[1]),
           get_node_char(state->current_nodes[2]),
           get_node_char(state->current_nodes[3]), state->keys_bitmask);
  }

  for (unsigned i = 0; i < next_steps.size; i++) {
    const struct robot_step *step = &next_steps.data[i];
    unsigned key_index = get_key_index(step->node);
    assert(0 <= key_index && key_index <= MAX_KEY_COUNT);

    debugf("Step %c -> %c (dist = %d)\n",
           get_node_char(state->current_nodes[step->robot]),
           get_node_char(step->node), step->dist);

    context->current_path.steps[context->current_path.len++] = *step;

    // Copy existing state with adjustments
    struct search_state next_state = {
        .total_dist = state->total_dist + step->dist,
        .keys_bitmask = state->keys_bitmask | (1 << key_index),
    };
    memcpy(&next_state.current_nodes, state->current_nodes,
           sizeof(next_state.current_nodes));
    next_state.current_nodes[step->robot] = step->node;

    find_best_path(context, &next_state);
...
void part2(const struct maze *input) {
  // Alter maze to have 4 doors
  struct maze altered_maze;
  memcpy(&altered_maze, input, sizeof(altered_maze));

  struct coord midpoint = {
      .row = altered_maze.rows / 2,
      .col = altered_maze.cols / 2,
  };

  // if (altered_maze.grid[midpoint.row][midpoint.col] != CELL_DOOR) {
  //   die("Expected door at middle of maze\n");
  // }

  altered_maze.grid[midpoint.row - 1][midpoint.col - 1] = '1';
  altered_maze.grid[midpoint.row - 1][midpoint.col] = CELL_WALL;
  altered_maze.grid[midpoint.row - 1][midpoint.col + 1] = '2';
  altered_maze.grid[midpoint.row][midpoint.col - 1] = CELL_WALL;
  altered_maze.grid[midpoint.row][midpoint.col] = CELL_WALL;
  altered_maze.grid[midpoint.row][midpoint.col + 1] = CELL_WALL;
  altered_maze.grid[midpoint.row + 1][midpoint.col - 1] = '3';
  altered_maze.grid[midpoint.row + 1][midpoint.col] = CELL_WALL;
  altered_maze.grid[midpoint.row + 1][midpoint.col + 1] = '4';

  print_maze(&altered_maze);

  struct graph graph;
  build_graph(&graph, &altered_maze);
  print_graph(&graph);
